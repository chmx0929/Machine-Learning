...
重点说一下用户社交行为的Embedding，因为用户社交行为是广告/推荐系统中极有价值的特征，尤其在微博这样的偏社交的平台下。难点也显而易见，用户社交行为数量巨大。这里我基于上一篇的算法进行优化：1. 加入时序影响（时间越近的权值越高）；2. 加入attention matrix（模型学习高影响力node/link），作为社交行为Embedding方法。照例也讨论一下社交特征的处理思路，常规的套路分为两种：
...
现在有很多比较fancy的文章，但实际上能参考的不多，工业界数据过于巨大，十亿级的用户，千亿级的关注关系，转评赞行为更甚且增量巨大。所以我们结合工业界的几篇论文，根据自身特点进行了用户社交行为的Embedding。让我们来分析一下现状：
