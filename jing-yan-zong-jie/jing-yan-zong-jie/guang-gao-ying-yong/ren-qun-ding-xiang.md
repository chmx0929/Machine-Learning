...
最近负责广告人群定向\(兴趣/行业\)体系的重构，写本篇记录一下工作。人群兴趣标签可以说是用户画像中最重要的一环，既可直接商业化又能作为基础特征数据输入后续算法。但用于商业化的人群兴趣标签和UGC\(用户原创内容\)标签的差别比较大\(比如商业化标签多采用广告导流人群等作为正样本等\)。且商业标签是给广告主用的，都是与广告商业价值强相关的类别，广告主进行选择的时候不需要过多思考即知道哪个标签是为自己广告计划量身定做的，追求的是提升广告效果。总的来说蛮有趣的，很值得去深入探讨和挖掘。
...
再让我们回到一开始说的游戏标签，猜测一下巨量明知不合理，但为什么还这么做呢？对于商业标签来说最最重要的一个原则：**听金主的**。下图是百科的分类，可以代表一般人认知了。巨量在此基础上加了消除、卡牌、解谜、跑酷、棋类，可能是这几个类有大的广告主来，故而又开发出这几类。所以用于广告的兴趣标签画像体系，合理只是基础原则，根本还是给广告主服务。但个人建议，像这种额外加的标签，只曝光给对应广告主即可，全部广告主都能看到容易造成误解歧义。
...
论文层出不穷，新的深度学习模型，知识图谱应用...无非是在探寻如何将一些我们认为是强相关的特征加入进去或者模型自己能学到哪些是强相关的特征，学习到更好的特征到类别的映射函数，比如加attention是为了像弹钢琴一样有轻有重抓重点；用RNN为了引入时序特征；将图片、文字、音频映射成向量为了引入多模态信息...特征工程做得好，不用很复杂fancy的模型也可以做出很好的效果。

![Embedding&amp;MLP structure](../../../.gitbook/assets/embedding-and-mlp.png)

由于也是采用的类Embedding&MLP框架，所以先将特征Embedding生成向量，再输入进模型，模型可以是经典机器学习模型，也可能是深度学习模型。所以我们先想办法把特征搞成向量，在数字化/量化特征的同时，尽量保留时序、多模态等等信息...
...
重点说一下用户社交行为的Embedding，因为用户社交行为是广告/推荐系统中极有价值的特征，尤其在微博这样的偏社交的平台下。难点也显而易见，用户社交行为数量巨大。照例也讨论一下社交特征的处理思路，常规的套路分为两种：
...
Graph Embedding现在有很多很好的方法，比如[EGES](https://arxiv.org/pdf/1803.02349.pdf)，[GraphSA](http://snap.stanford.edu/graphsage/)，[GCN](https://arxiv.org/pdf/1609.02907.pdf)等，非常有参考价值，但场景略有不同。我们有关注、转发、评论、点赞\(还有收藏等等\)，至少四种维度的link关系；有账号、博文等至少两种重要类型node，是异构的。当然可以把博文信息融合到账号节点中，然后四种维度link关系当成相同链接，只不过给不同的链接权重。但这种融合会丧失部分信息，计算时间在十亿量级node万亿量级link上过于耗时，而且融合权重需要额外计算。所以这里我取了个时间窗内的社交行为作为数据，基于我上一篇的算法进行优化：1. 加入时序影响（时间越近的权值越高）；2. 加入attention matrix（模型学习高影响力node/link），作为社交行为Embedding方法。
...
针对我们的场景，先试探性给用户打上兴趣标签，根据其对相应UGC内容和广告的反馈，调节之前的兴趣标签，各标签试探概率随着时间逐渐降低，最终趋于稳定。不同于随机生成初始解，我们预先生成了不同用户类的初始兴趣标签概率解，这里的用户类基于可拿到的维度数据，比如 $$\text{User}\text{(beijing,\ ios, afternoon)}$$ 初始都是 $$[0.6,0.8,0.4]$$ ，其中每维代表购物、游戏、教育。
