...
最近负责广告人群定向\(兴趣/行业\)体系的重构，写本篇记录工作。人群兴趣标签可以说是用户画像中最重要的一环，既可直接商业化又能作为基础特征数据输入后续算法。但用于商业化的人群兴趣标签和UGC\(用户原创内容\)标签的差别比较大\(比如商业化标签多采用广告导流人群等作为正样本等\)。且商业标签是给广告主用的，都是与广告商业价值强相关的类别，广告主进行选择的时候不需要过多思考即知道哪个标签是为自己广告计划量身定做的，追求的是提升广告效果。总的来说蛮有趣的，很值得去深入探讨和挖掘
