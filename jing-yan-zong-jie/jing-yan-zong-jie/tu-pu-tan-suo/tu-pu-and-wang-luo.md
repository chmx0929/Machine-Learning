...
Small-word性质\(6度理论\)适用于 $$U$$ 集合空间。对于 $$V$$ 集合空间来说，物料属性是固定的，可根据不同粒度/维度属性将 $$v\in V$$关联起来，比如 华为p40 - 苹果iPhoneX\(手机\)，苹果iPhoneX - 苹果AirPods\(品牌\)...

那么对于 $$U+V$$ 空间来说：

* Local：原本 $$u_a -u_b-u_c \to v_i$$ ，表示用户 $$u_a$$ 到 $$v_i$$ 的距离，Local平均路径降低后， $$u_a$$ 可能直接和 $$u_c$$ 建立链接，即变为 $$u_a - u_c \to v_i$$ 。即闺蜜从李佳琦那边了解到某色号推荐给我女朋友，现在我女朋友直接看李佳琦直播间...
* Global：原本 $$u_a -u_b-u_c \to v_i$$，表示用户 $$u_a$$ 到 $$v_i$$ 的距离，Global平均路径降低后， $$u_b$$ 可能直接和 $$v_i$$ 建立链接，即变为 $$u_a - u_b \to v_i$$ 。即淘宝今年GMV再创新高，客单价不变的情况下\(用户群固定即消费能力固定\)，每人购买的东西多了， 两集合空间链接多了。



#### Clustering effect

对于 $$U$$ 集合空间，存在账号实际相邻关系\(比如相互关注链接\)，但这个实际相邻关系只是我们想要结构的一部分，可理解为Local Structure。还有Global Structure，比如通过 $$u_a - u_b \to v_k$$ 路径给用户 $$a$$ 推物料 $$k$$ ，为什么给他推 $$k$$ 呢？因为和他相似的用户 $$b$$ 和物料 $$k$$ 有链接，这里的 $$u_a - u_b$$ 链接即Global Structure，用户 $$a$$ 和 $$b$$ 现实中没有联系，只是我们计算出了他们在 $$U$$ 集合空间中是相似的。
