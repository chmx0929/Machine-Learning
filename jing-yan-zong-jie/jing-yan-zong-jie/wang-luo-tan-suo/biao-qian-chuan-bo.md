...
账号 $$x$$ 结果通过其粉丝集合 $$A$$求和，然后经过标准化得到，标准化 $$\sigma$$ 函数任选，比如softmax，数据量太大可以直接计算对应值除以总和得到占比。当然，如果像进行用户兴趣分类，这是个多标签任务，而非多分类任务，可以采用1：所有账号同一标签的max值作为分母，各账号此标签权重作为分子; 2：同一账号各标签的max权重作为分母，账号各标签权重作为分子。有这两个概率后排序，找top的重合部分进行标注。
