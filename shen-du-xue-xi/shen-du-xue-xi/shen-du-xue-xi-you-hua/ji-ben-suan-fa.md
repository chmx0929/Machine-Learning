...
下图为代价函数等高线，以及不同优化算法随时间变化。其中可以明确看到，基于动量的算法看上去像一个小球从山峰向下滚。

![](../../../.gitbook/assets/opt2.gif)

下图是各优化算法在鞍点时的表现。可以看到SGD很难打破对称性并且困在了顶端。而像RMSprop等可以较快找到方向。
...
第三个方程是更新操作，超参数 $$\rho$$ 通常选为 $$0.9$$ ，但是你可能需要调整它。方程 2 中的 $$\epsilon$$ 是为了防止被 $$0$$ 除，通常取 $$e^{-10}$$ 。
...
我们计算了每个梯度分量的指数平均和梯度平方指数平均（方程 1、方程 2）。为了确定迭代步长我们在方程 3 中用梯度的指数平均乘学习率（如 Momentum 的情况）并除以根号下的平方指数平均（如 Momentum 的情况），然后方程 4 执行更新步骤。超参数 $$\beta_1$$ 一般取 $$0.9$$ 左右， $$\beta_2$$ 取 $$0.99$$ 。 $$\epsilon$$ 一般取 $$e^{-10}$$ 。
