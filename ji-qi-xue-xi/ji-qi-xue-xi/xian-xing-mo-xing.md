...
对数几率函数是一种“Sigmoid函数”，它将 $$z$$ 值转化为一个接近 $$0$$ 或 $$1$$ 的 $$y$$ 值，并且其输出值在 $$z = 0$$ 附近变化很陡，将对数几率函数作为联系函数 $$g^{-1}(\cdot)$$ ，得到
                                           ...
                                           $$\Rightarrow \frac{y}{1-y}= \frac{\frac{1}{1+e^{-(w^Tx+b)}}}{1-\frac{1}{1+e^{-(w^Tx+b)}}}=\frac{1}{1+e^{-(w^Tx+b)}-1}=\frac{1}{e^{-(w^Tx+b)}}$$ 

                                           $$\Rightarrow \ln \frac{y}{1-y} = \ln \frac{1}{e^{-(w^Tx+b)}} = \ln e^{(w^Tx+b)} =w^Tx+b $$ 
