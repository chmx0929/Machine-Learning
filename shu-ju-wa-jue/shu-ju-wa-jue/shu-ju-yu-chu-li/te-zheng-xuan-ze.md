...
$$L_1$$ 范数和 $$L_2$$ 范数正则化都有助于降低过拟合风险，但前者还会带来一个额外的好处： $$L_1$$ 比 $$L_2$$ 更易获得“稀疏”解，即它求得的 $$w$$ 会有更少的非零分量。比如如下例子：假定 $$x$$ 仅有两个属性，于是无论岭回归还是LASSO解出的 $$w$$ 都只有两个分量，即 $$w_1,w_2$$ ，我们将其作为两个坐标轴，然后在途中绘制出第一项的“等值线”，即在 $$(w_1,w_2)$$ 空间中平方误差取值相同的点的连线，再分别绘制出 $$L_1$$ 范数与 $$L_2$$ 范数的等值线，即在 $$(w_1,w_2)$$ 空间中 $$L_1$$ 范数取值相同的点的连线，以及 $$L_2$$ 范数取值相同的点的连线。
