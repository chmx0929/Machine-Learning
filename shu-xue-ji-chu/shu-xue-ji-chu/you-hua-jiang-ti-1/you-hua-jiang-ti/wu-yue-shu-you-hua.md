...
代码如下\(求平方根，即 $$m = 2$$ ， $$x_{n+1} = (1-\frac{1}{2})x_n+\frac{a}{2x_n^{2-1}}=\frac{1}{2}(x_n+\frac{a}{x_n})$$ \)：
...
假设我们求解 $$f(x)$$ 的极小值，其中 $$x=(x_1,x_2,\dots,x_d)^T\in \mathbb{R}^d$$ 是一个 $$d$$ 维向量。从初始点 $$x^0$$ 开始，坐标下降法通过迭代地构造序列 $$x^0,x^1,x^2,\dots$$ 来解决问题， $$x^{t+1}$$ 的第 $$i$$ 个分量 $$x_i^{t+1}$$ 构造为：
                               ...
                               $$x_i^{t+1}=\mathop{\arg \min}\limits_{y\in\mathbb{R}}f(x_1^{t+1},\dots,x_{i-1}^{t+1},y,x^t_{i+1},\dots,x_d^t)$$ 
